...
<nav class="mb-6 text-sm" aria-label="Breadcrumb">
    <ol class="flex flex-wrap gap-2 text-gray-500">
        ...
        <li><a href="{{ route('browse.index') }}" class="hover:text-amber-400 transition-colors">Browse</a></li>
        <li class="text-gray-600">/</li>
        <li class="text-amber-400">{{ $tag->name }}</li>
    </ol>
</nav>

<div class="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-4 mb-10">
    <div>
        <h1 class="text-3xl md:text-4xl font-bold text-white font-serif">{{ $tag->name }} casinos</h1>
        <p class="text-gray-500 mt-1">Sort and compare licensed listings.</p>
    </div>
...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
    ...
    @empty
        <p class="col-span-full text-gray-500">No casinos with this tag yet.</p>
    @endforelse
</div>
